...
import { ChevronDown, ChevronRight, Plus, Search, SlidersHorizontal, Trash2 } from "lucide-react";
...
const SORT_OPTIONS = [["due", "Fecha límite"], ["priority", "Prioridad"], ["status", "Estado"], ["assignee", "Responsable"], ["project", "Proyecto"], ["created", "Fecha de creación"], ["title", "Nombre"]];
...
export default function WorkspaceOperations({ tasks, projects, sections, statuses, members, units, activeUnitId, storageKey, pending, loading, permissionsCan, onOpenTask, onEditTask, onOpenCreate, onQuickCreate, onBulk }) {
    ...
    useEffect(() => { const close = event => { const panel = filtersRef.current; if (!panel?.open) return; if (event.type === "keydown" && event.key === "Escape") panel.open = false; if (event.type === "pointerdown" && !panel.contains(event.target)) panel.open = false; }; document.addEventListener("keydown", close); document.addEventListener("pointerdown", close); return () => { document.removeEventListener("keydown", close); document.removeEventListener("pointerdown", close); }; }, []);
    ...
    const saveView = () => { const name = window.prompt("Nombre de la vista"); if (!name?.trim()) return; persistViews([...views.filter(view => view.name !== name.trim()), { name: name.trim(), filters, sort, direction, group, fields }]); };
    const applyView = view => { setFilters(view.filters || emptyWorkspaceFilters()); setSort(view.sort || "due"); setDirection(view.direction || "asc"); setGroup(view.group || ""); setFields(view.fields || DEFAULT_FIELDS); setSelected([]); };
    ...
    return <section className="workspace_ops" aria-label="Gestión de tareas de BOLD Workspace">
        ...
        {allowed.create && <form className="workspace_quick_add" onSubmit={quickCreate}><input id="workspace-quick-title" aria-label="Nombre de la nueva tarea" placeholder="+ Agregar tarea rápidamente" value={quickParent ? "" : quickTitle} onChange={event => { setQuickParent(null); setQuickTitle(event.target.value); }} onKeyDown={event => { if (event.key === "Escape") setQuickTitle(""); }} /><button type="submit" disabled={pending || !quickTitle.trim()}>Crear</button></form>}
        {modal && <div className="workspace_modal_overlay" onPointerDown={event => { if (event.target === event.currentTarget) setModal(""); }}><form className="workspace_modal" role="dialog" aria-modal="true" aria-label={modal === "subtasks" ? "Crear subtareas para tareas seleccionadas" : "Crear varias tareas"} onSubmit={modal === "subtasks" ? submitSubtasks : batchCreate}><header><h2>{modal === "subtasks" ? "Subtareas para tareas seleccionadas" : batchParent ? "Crear varias subtareas" : "Crear varias tareas"}</h2><button type="button" aria-label="Cerrar" onClick={() => setModal("")}>×</button></header><p className="workspace_batch_preview">{modal === "subtasks" ? `${batchText.split(/\r?\n/).filter(item => item.trim()).length * selectedIds.length} subtareas en ${selectedIds.length} tareas` : `${batchText.split(/\r?\n/).filter(item => item.trim()).length} tareas para crear`}</p><label>Una tarea por línea<textarea autoFocus rows="8" value={batchText} onChange={event => setBatchText(event.target.value)} required /></label>{modal !== "subtasks" && <><label>Proyecto<select value={batchProject} onChange={event => { setBatchProject(event.target.value); setBatchSection(""); }}><option value="">Sin proyecto</option>{projectOptions}</select></label><label>Sección<select value={batchSection} onChange={event => setBatchSection(event.target.value)}><option value="">Sin sección</option>{sections.filter(item => String(item.projectId || item.project) === batchProject).map(item => option(item.id, item.label || item.name))}</select></label><label>Responsable<select value={batchAssignee} onChange={event => setBatchAssignee(event.target.value)}><option value="">Sin responsable</option>{memberOptions}</select></label><label>Fecha límite<input type="date" value={batchDate} onChange={event => setBatchDate(event.target.value)} /></label></>}<label>Prioridad<select value={batchPriority} onChange={event => setBatchPriority(event.target.value)}>{PRIORITIES.map(([id, label]) => option(id, label))}</select></label><footer><button type="button" onClick={() => setModal("")}>Cancelar</button><button type="submit" className="primary_button" disabled={pending}>Crear</button></footer></form></div>}
    </section>;
}
