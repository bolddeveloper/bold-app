...
export default function WorkspaceOperations({ TaskSelect, CalendarDateField, tasks, projects, sections, statuses, members, units, activeUnitId, storageKey, pending, loading, permissionsCan, onOpenTask, onOpenCreate, onBulk }) {
    ...
    const headerFilter = field => {
        const configs = {
            assignee: ["assignees", members.map(item => [String(item.id), item.name])],
            project: ["projects", projects.map(item => [String(item.id), item.label || item.name])],
            section: ["sections", sections.map(item => [String(item.id), item.label || item.name])],
            priority: ["priorities", PRIORITIES],
            status: ["statuses", statuses.map(item => [String(item.id), item.label || item.name])],
        };
        if (field === "due") return <Select aria-label="Filtrar por fecha límite" value={filters.due} onChange={event => setFilters(current => ({ ...current, due: event.target.value, dueFrom: "", dueTo: "" }))}>{option("", "Fecha límite")}{DATE_OPTIONS.slice(1).map(([id, label]) => option(id, label))}</Select>;
        const config = configs[field];
        if (!config) return FIELD_LABELS[field];
        const [key, options] = config;
        return <Select aria-label={`Filtrar por ${FIELD_LABELS[field]}`} value={filters[key][0] || ""} onChange={event => setFilters(current => ({ ...current, [key]: event.target.value ? [event.target.value] : [] }))}><option value="">{FIELD_LABELS[field]}</option>{options.map(([id, label]) => option(id, label))}</Select>;
    };
    const row = (task, level = 0) => {
        ...
        </div>{isExpanded && children.map(child => row(child, level + 1))}
        </Fragment>;
    };
    ...
    return <section className="workspace_ops" aria-label="Gestión de tareas de BOLD Workspace">
        ...
        <div className="workspace_toolbar"><label className="workspace_search"><Search size={17} /><input type="search" placeholder="Buscar tareas, descripción o etiquetas" value={query} onChange={event => setQuery(event.target.value)} /></label><details ref={filtersRef}><summary><SlidersHorizontal size={17} /> Filtros {activeChips.length ? `(${activeChips.length})` : ""}</summary><div className="workspace_filter_panel"><button type="button" className="workspace_filters_close" onClick={event => { event.currentTarget.closest("details").open = false; }}>Cerrar filtros</button>
            ...
            <label>Desde<CalendarDateField value={filters.dueFrom} onChange={value => setFilter("dueFrom", value)} /></label><label>Hasta<CalendarDateField value={filters.dueTo} onChange={value => setFilter("dueTo", value)} /></label>
            <label>Creada desde<CalendarDateField value={filters.createdFrom} onChange={value => setFilter("createdFrom", value)} /></label><label>Creada hasta<CalendarDateField value={filters.createdTo} onChange={value => setFilter("createdTo", value)} /></label>
        ...
        {selectedIds.length > 0 && <div className="workspace_bulk" role="region" aria-label="Acciones masivas">
            ...
            {allowed.update && <>
                ...
                <label>Fecha<CalendarDateField onChange={value => { if (value) bulkUpdate("due_date", value); }} /></label>
            </>}
