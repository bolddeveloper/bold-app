...
test("Core imports no module internals and local frontend imports have no cycles", () => {
    const sources = files(root).filter(file => !file.endsWith(".test.js"));
    ...
    for (const [file, imports] of graph) {
        if (file.startsWith(path.join(root, "core") + path.sep)) for (const target of imports) assert.ok(target.startsWith(path.join(root, "core") + path.sep) || target.startsWith(path.join(root, "shared") + path.sep), `${file} imports ${target}`);
    }
