...
try {
 ...
 root.unmount();
 if (real) { const api=(await server.ssrLoadModule('/@fs/' + path.resolve('../tareas/src/services/tasks_api.js').replaceAll('\\', '/'))).api; if((await server.ssrLoadModule("/@fs/" + path.resolve("../core/http_client.js").replaceAll("\\", "/"))).http.getSession().token) for(const task of await api.listTasks()) if([testTitle,childTitle].includes(task.title)) await api.deleteTask(task.id); }
