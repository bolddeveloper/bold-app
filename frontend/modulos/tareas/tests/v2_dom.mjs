...
try {
 ...
 const api=(await server.ssrLoadModule('/@fs/' + path.resolve('../tareas/src/services/tasks_api.js').replaceAll('\\', '/'))).api; for (const task of await api.listTasks()) if(task.title===testTitle) await api.deleteTask(task.id); window.dispatchEvent(new window.Event('focus')); await pause();
 ...
 if(real) { const api=(await server.ssrLoadModule('/@fs/' + path.resolve('../tareas/src/services/tasks_api.js').replaceAll('\\', '/'))).api; for(const task of await api.listTasks()) if(task.title===childTitle) await api.deleteTask(task.id); }
 for(const label of ['Inicio','Informes']) { button(label).click(); await pause(); }
 console.log('DOM PASS: home/reports render');
 assert.deepEqual(errors,[]);
} finally {
 root.unmount();
 if (real) { const api=(await server.ssrLoadModule('/@fs/' + path.resolve('../tareas/src/services/tasks_api.js').replaceAll('\\', '/'))).api; if((await server.ssrLoadModule("/@fs/" + path.resolve("../core/http_client.js").replaceAll("\\", "/"))).http.getSession().authenticated) for(const task of await api.listTasks()) if([testTitle,childTitle].includes(task.title)) await api.deleteTask(task.id); }
